...
def test_server_stpmtr_superuser(server_test: Server, superuser_test: SuperUser, stpmtr_emulate_test: StpMtrCtrl_emulate):
    ...
    for device_id, device in devices.items():
        if device_id != server.id:
            msg: MessageExt = device.thinker.tasks_in_test.popitem()[1]
            assert msg.com == MsgComExt.HEARTBEAT_FULL.msg_name
            assert msg.sender_id == server.id
...
#     sleep(0.02)
#     # server must get all of these demands and react_forward them to services
