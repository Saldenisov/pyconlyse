...
T14 device API construction focused checks:

```bash
conda run -n pyconlyse39 python -m pytest --strict-config --deny-network \
  tests/web/test_tango_gateway.py \
  tests/web/test_device_api_smoke.py \
  tests/web/test_device_api_psp.py
```

Required cases: every database/proxy construction in `device_api.py` delegates
...
T16 read-only DAQmx/PSP catalog focused checks:

```bash
conda run -n pyconlyse39 python -m pytest --strict-config --deny-network \
  tests/web/test_device_catalog_service.py \
  tests/web/test_device_api_psp.py
```

Required cases: the catalog service imports no Tango or web framework; DAQmx
retains class query order, per-class failure skipping, first-match
de-duplication, state fallback, and lexical sorting; PSP retains duplicates,
default state probe, fallback rows, and HTTP payload shape. Named-module
coverage requires 100% statements for `web/backend/device_catalog_service.py`.
...
suites/70 tests passed. Current full Python deny-network gate: tooling 41
passed; Python 600 passed, 1 skipped; frontend 10 suites/83 tests; frontend
...
Current full Python deny-network evidence: tooling 41 passed; Python 600
passed, 1 skipped, 18 warnings; named-module coverage 69.0%; frontend 10
