...
class DS_GPIO(DS_General):
    ...
    RULES = {
        "set_pin_state": [DevState.ON, DevState.STANDBY],
        "get_pin_state": [DevState.ON, DevState.STANDBY],
        **DS_General.RULES,
    }
    ...
    @command(
        dtype_in=[int],
        doc_in="In [Pin_id, value] function that changes state of outputs of the controller.",
        display_level=DispLevel.OPERATOR,
    )
    def set_pin_state(self, pins_values: List[int]):
        ...
        if state_ok == 1:
            ...
            if res != 0:
                self.error(
                    f"Setting pin {pins_values[0]} value {pins_values[1]} of device {self.device_name} was NOT "
                    f"accomplished with success: {res}"
                )

    @command(
        dtype_in=int,
        doc_in="In Pin_id function that changes state of outputs of the controller.",
        display_level=DispLevel.OPERATOR,
        dtype_out=int,
    )
    def get_pin_state(self, pin_id: int):
        ...
        if state_ok == 1:
            ...
            if isinstance(res, str):
                self.error(
                    f"Getting pin {pin_id} value of device {self.device_name} was NOT "
                    f"accomplished with success: {res}"
                )
                res = -1
        return res
    ...
    @command(
        dtype_in=str,
        doc_in="Pass the name of order",
        doc_out="Give the order pulses finished",
        dtype_out=int,
    )
