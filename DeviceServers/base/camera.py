...
from DeviceServers.base.general import GeneralOrderInfo
...
class DS_CAMERA(DS_General):
    ...
    def register_order_local(self, name, value):
        order_info = OrderInfo(
            order_length=value[0],
            order_done=False,
            order_timestamp=time(),
            ready_to_delete=False,
            order_array=np.array([self.wavelengths]),
        )
            ...
            if self.orders:
                ...
                for order_name, order_info in self.orders.items():
                    if (time() - order_info.order_timestamp) >= 100:
                        ...
                        order_info.order_array = np.vstack(
                            [order_info.order_array, spectrum]
                        )
