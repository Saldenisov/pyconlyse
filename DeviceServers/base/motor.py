...
from DeviceServers.base.general import DS_General, standard_str_output
            ...
            for ds_id, ds_param in self._delay_lines_parameters.items():
                for param_name, param_type in must_have.items():
                    if param_name not in ds_param.keys():
                        raise KeyError(
                            f"{param_name} is not {ds_param} of axis {ds_id}."
                        )
                    if not isinstance(ds_param[param_name], param_type):
                        raise TypeError(
                            f"{ds_param[param_name]} is a wrong type {type(ds_param[param_name])}, "
        ...
        if state_ok == 1:
            ...
            if res != 0:
                try:
                    ...
                    axis = None
                self.error(
                    f"Could not define position for axis {axis} of {self.device_name}: {res}"
                )
        else:
