...
from DeviceServers.base.DS_GPIO import DS_GPIO, MyPin, OrderPulsesInfo
from utilities.myfunc import get_random_string, ping
    ...
    def find_device(self):
        ...
        if state_ok:
            if ping(self.ip_address):
                ...
                self._device_id_internal, self._uri = (
                    int(self.device_id),
                    self.friendly_name.encode("utf-8"),
                )
                states = []
...
class MyPinRPI(MyPin):
    def __init__(
        self, pin_number: int, parameters: tuple, ip_address: str, parent: DS_RPI_GPIO
    ):
        ...
        if parameters[3] == "gpiozero":
            ...
            self.pin: LED = eval(parameters[0])(
                pin=self.pin_number, pin_factory=factory
            )
            self.on = self.pin.on
    ...
    def ttl_zmq(self, order: OrderPulsesInfo):
        ...
        msg = {
            ...
            "order_id": get_random_string(10),
            "pin_number": self.pin_number,
            "number_ttl": order.number_of_pulses,
            "delay": order.time_delay,
            "width": order.dt,
        }
        msg = str(msg)
        self.send_msg(msg)
    ...
    def treat_msg(self, msg):
        if msg:
            ...
            if self.order:
                ...
                if (
                    self.order.number_of_pulses == self.order.pulses_done
                    and not self.order.order_done
                ):
