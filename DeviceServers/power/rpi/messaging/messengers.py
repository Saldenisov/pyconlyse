...
from utilities.errors.messaging_errors import MessageError, MessengerError
from utilities.errors.myexceptions import ThinkerError, WrongAddress
from utilities.myfunc import error_logger, get_free_port, get_local_ip, info_msg
...
class Messenger(MessengerInter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.name = (
            f"{self.__class__.__name__}:{Messenger.n_instance}:{name}:{get_local_ip()}"
        )
        self.id: DeviceId = parent.id
        self.parent: Device = parent
        try:
            self._polling_time = (
                int(self.parent.get_general_settings()["polling"]) / 1000.0
            )
        except AttributeError:
            self._polling_time = 1
        self.active = False
        self.paused = True
        self._msg_out = MsgDict(
            name=f"msg_out:{self.id}", size_limit=1000, dict_parent=self
        )
    ...
    def decrypt_with_session_key(
        self, msg_bytes: bytes, device_id: DeviceId = None
    ) -> str:
        # TODO: add functionality to messenger.decrypt() when TLS is realized
        try:
            return Fernet(self.parent.connections[device_id].session_key).decrypt(
                msg_bytes
            )
        except KeyError:
            raise MessengerError("DeviceID is not known, cannot decrypt msg")
    ...
    def encrypt_with_public(self, msg_b: bytes, pem: bytes) -> bytes:
        ...
        msg_encrypted = public_key.encrypt(
            msg_b,
            padding.OAEP(
                mgf=padding.MGF1(algorithm=hashes.SHA1()),
                algorithm=hashes.SHA1(),
    ...
    def encrypt_with_session_key(self, msg: MessageExt) -> bytes:
        ...
        if msg.crypted:
            try:
                ...
                return Fernet(self.parent.connections[receiver_id].session_key).encrypt(
                    msg.byte_repr(compression=False)
                )
            except KeyError:
                raise MessengerError("DeviceID is not known, cannot encrypt msg")
        else:
    ...
    def run(self):
        ...
        self._send_loop_logic(await_time=0.1 / 1000.0)
...
class ClientMessenger(Messenger):
    ...
    def bind_pub(self):
        try:
            self.sockets[PUB_Socket].setsockopt_unicode(
                zmq.IDENTITY, self.addresses[PUB_Socket]
            )
        ...
        for msg, device_id, socket, crypted in msgs:
            try:
                if int(crypted):
                    ...
                    )  #  !!! ONLY WHEN CLIET/SERVICE HAS DEALER SOCKET
                ...
                msg_r = self.parent.generate_msg(
    ...
    def _receive_msgs(self):
        ...
        while self.active:
            try:
                ...
                ValueError
            ) as e:  # TODO when recv_multipart works wrong! what will happen?
                self.logger.error(e)
    ...
    def _verify_addresses(self, addresses: Dict[str, str]):
        ...
        local_ip = get_local_ip()
        self.addresses[PUB_Socket] = f"tcp://{local_ip}:{port}"
        self.addresses[PUB_Socket_Server] = (
            addresses[PUB_Socket_Server].replace(" ", "").split(",")
        )

    def _wait_server_hb(self) -> MessageExt:
        if (
            FRONTEND_Server not in self.addresses
            or BACKEND_Server not in self.addresses
        ):
            wait = True
        else:
            wait = False
                    ...
                    try:
                        ...
                        if FRONTEND_Server in sockets and BACKEND_Server in sockets:
                            ...
                            info_msg(
                                self,
                                "INFO",
                                "Info from Server is obtained for messenger operation.",
                            )
...
class ServerMessenger(Messenger):
    ...
    def _create_sockets(self):
        try:
            ...
            self.public_sockets = {
                PUB_Socket_Server: self.addresses[PUB_Socket_Server],
            ...
            if not self.paused:
                ...
                try:
                    if self.sockets[FRONTEND_Server] in sockets:
                        device_id, msg, crypted = self.sockets[
                            FRONTEND_Server
                        ].recv_multipart()
    ...
    def send_msg(self, msg: MessageExt):
        try:
            ...
            elif msg.receiver_id in self._frontendpool:
    ...
    def _verify_addresses(self, addresses: Dict[str, str]):
        if not isinstance(addresses, dict):
            raise Exception(
                f"{addresses} are not dict, func: {self._verify_addresses.__name__} of {self.parent.name}"
            )
