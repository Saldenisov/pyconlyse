...
from communication.messaging.message_types import (
    MessageInfoExt,
    MessageInfoInt,
    MsgType,
)
...
class MsgComExt(Enum):
    AVAILABLE_SERVICES = MessageInfoExt(
        MsgType.DIRECTED, AvailableServices, set(["available_services"]), True
    )
    DO_IT = MessageInfoExt(
        MsgType.DIRECTED, DoIt, set(["receiver_id", "func_input"]), True
    )
    DONE_IT = MessageInfoExt(
        MsgType.DIRECTED, DoneIt, set(["receiver_id", "reply_to", "func_output"]), True
    )
    ERROR = MessageInfoExt(
        MsgType.DIRECTED, MsgError, set(["comments", "reply_to", "receiver_id"]), False
    )
    HEARTBEAT = MessageInfoExt(MsgType.BROADCASTED, HeartBeat, set(["event"]), False)
    HEARTBEAT_FULL = MessageInfoExt(
        MsgType.BROADCASTED, HeartBeatFull, set(["event"]), False
    )
    SHUTDOWN = MessageInfoExt(MsgType.BROADCASTED, ShutDown, set(["reason"]), False)
    WELCOME_INFO_DEVICE = MessageInfoExt(
        MsgType.DIRECTED, WelcomeInfoDevice, set(["receiver_id", "event"]), False
    )
    WELCOME_INFO_SERVER = MessageInfoExt(
        MsgType.DIRECTED, WelcomeInfoServer, set(["reply_to", "receiver_id"]), False
    )
...
class MessageExt(Message):
    ...
    def short(self):
        ...
        if not self.forwarded_from:
            return {
                "path": f"{self.sender_id}->{self.receiver_id}",
                "datastructures": f"{self.com}: {t[0:l]}...",
                "reply_to": self.reply_to,
                "id": self.id,
            }
        return {
            "path": f"{self.forwarded_from}->{self.sender_id}->{self.receiver_id}",
            "datastructures": f"{self.com}: {t[0:l]}...",
            "reply_to": self.reply_to,
            "id": self.id,
        }
    ...
    def bytes_to_msg(mes_bytes: bytes, coding: Coding = Coding.JSON) -> Message:
        if (
            coding is Coding.MSGPACK
        ):  # MSGPACK is not realy working for all type of messages
            try:
                ...
                raise MessageError(
                    f'Error: "{e}" in bytes_to_msg coding {coding}`. Msg={mes_bytes}'
                )
        elif coding is Coding.JSON:
            try:
            ...
            module_logger.info(
                f"{Coding.MSGPACK} is not realy working for all types of messages"
            )
            raise MsgError(
                f"Wrong coding type passed {coding}. Choose between {Coding.JSON} and {Coding.MSGPACK}"
            )
