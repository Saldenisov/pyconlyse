...
class OWIS_motor(DS_General_Widget):
    def __init__(self, device_name: str, axes, parent=None, vis_type=VisType.FULL):
        ...
        ds.subscribe_event(
            "positions", tango.EventType.CHANGE_EVENT, self.positions_listener
        )
    ...
    def register_DS_full(self, group_number=1):
        ...
        delay_lines_parameters = ds.get_property("delay_lines_parameters")[
            "delay_lines_parameters"
        ][0]
        ...
        for axis, widget in zip(self.axes, widgets):
            ...
            lab_name.customContextMenuRequested.connect(
                partial(self.context_menu_label, axis)
            )
            setattr(self, f"label_name_{axis}_{dev_name}", lab_name)
            lab_name.clicked.connect(partial(self.label_name_clicked, axis))

            lab_name_relative_shift = MyQLabel(
                f"Relative shift: {self.delay_lines_parameters[axis]['relative_shift']}"
            )
            setattr(
                self,
                f"lab_name_relative_shift_{axis}_{dev_name}",
                lab_name_relative_shift,
            )

            setattr(
                self,
                f"radio_button_group_{axis}_{dev_name}",
                Qt.QGroupBox("Preset Positions"),
            )
            group_rb: Qt.QGroupBox = getattr(
                self, f"radio_button_group_{axis}_{dev_name}"
            )
            lo_rb_preset = QVBoxLayout()
            for rb in [
                Qt.QRadioButton(text=str(pos))
                for pos in self.delay_lines_parameters[axis]["preset_positions"]
            ]:
    ...
    def context_menu_label(self, axis, point):
        ...
        if action and self.axis_selected:
            ...
            label_shift: MyQLabel = getattr(
                self, f"lab_name_relative_shift_{self.axis_selected}_{self.dev_name}"
            )
            label_shift.setText(
                f"Relative shift: {self.delay_lines_parameters[self.axis_selected]['relative_shift']}"
            )
    ...
    def keyPressEvent(self, event: QtGui.QKeyEvent):
        ...
        if (
            event.key() in [Qt.Key_Left, Qt.Key_Down, Qt.Key_Right, Qt.Key_Up]
            and self.axis_selected
        ):
            positions = eval(ds.positions)
    ...
    def set_clicked(self, axis):
        ds: Device = getattr(self, f"ds_{self.dev_name}")
        lineedit: TaurusValueLineEdit = getattr(
            self, f"pos_lineedit{axis}_{self.dev_name}"
        )
        try:
    ...
    def positions_listener(self, event):
        ...
        for axis_id in self.delay_lines_parameters.keys():
            if self.positions[axis_id] != positions[axis_id]:
                ...
                label: QCheckBox = getattr(
                    self, f"pos_lab_name{axis_id}_{self.dev_name}"
                )
