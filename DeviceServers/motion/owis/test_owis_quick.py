#!/usr/bin/env python
"""
Quick test for OWIS PS90 Axis 2
Tests 4 predefined positions with extensive logging for debugging
"""
...
AXIS = 2
TEST_POSITIONS = [0.0, -15.0, -35.0, -50.0]  # Four test positions in mm
...
def main():
    log("=" * 70)
    log("=== OWIS PS90 Axis 2 Quick Test ===")
    log("=" * 70)
    log(f"Device: {DEVICE_NAME}")
    log(f"Axis: {AXIS}")
    log(f"Test positions: {TEST_POSITIONS}")
    log(f"Wait time at each position: {WAIT_TIME}s")
    log("")
    ...
    # Check axis state
    log(f"Checking axis {AXIS} state...")
    try:
        axis_state = device.get_status_axis(AXIS)
        log(f"✓ Axis {AXIS} state: {axis_state}", 1)
    except Exception as e:
        log(f"✗ WARNING: Could not get axis state: {e}", 1)
    
    log("")
    log("=" * 70)
    log(f"Starting test sequence with {len(TEST_POSITIONS)} positions...")
    log("=" * 70)
    log("")
    ...
    for i, target_pos in enumerate(TEST_POSITIONS, 1):
        log("-" * 70)
        log(f"MOVE {i}/{len(TEST_POSITIONS)}: Target position = {target_pos:.3f} mm")
        log("-" * 70)
        ...
        try:
            ...
            # Get axis state before move
            log("Checking axis state before move...", 1)
            try:
                axis_state_before = device.get_status_axis(AXIS)
                log(f"Axis state before move: {axis_state_before}", 2)
            except Exception as e:
                log(f"WARNING: Could not get axis state: {e}", 2)
            ...
            # Get axis state after move
            log("Checking axis state after move...", 1)
            try:
                axis_state_after = device.get_status_axis(AXIS)
                log(f"Axis state after move: {axis_state_after}", 2)
            except Exception as e:
                log(f"WARNING: Could not get axis state: {e}", 2)
    ...
    log("=== TEST COMPLETE ===")
    log("=" * 70)
    log(f"Total positions tested: {len(TEST_POSITIONS)}")
    log(f"Successful moves: {success_count}")
    log(f"Failed moves: {error_count}")
    log(f"Success rate: {100 * success_count / len(TEST_POSITIONS):.1f}%")
    log(f"Total time: {total_time:.1f}s ({total_time/60:.1f} minutes)")
    log(f"Average time per move: {total_time/len(TEST_POSITIONS):.1f}s")
    log("")
