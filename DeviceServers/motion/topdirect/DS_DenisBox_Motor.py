...
class DS_DenisBox_Motor(DS_MOTORIZED_MONO_AXIS):
    ...
    def find_device(self):
        ...
        if file_exists:
            with open(f"{self.friendly_name}_position.txt") as f:
                line = f.readline()
    ...
    def check_controlled_devices(self):
        active = {
            "enable_ds": self.enable_ds.state,
            "dir_ds": self.dir_ds.state,
            "pulse_ds": self.pulse_ds.state,
        }
        error = False
        errors = []
    ...
    def move_axis_local(self, pos) -> Union[int, str]:
        ...
        if microsteps != 0:
            ...
            order_name = self.pulse_ds.register_order(
                [self.pulse_pin, microsteps, self.dt, self.delay_time]
            )
            ...
            for i in range(120):
                ...
                if not ready:
                    ...
                    self._position += (
                        dir * (done - already_done) / self.microstep * self.step_mm
                    )
                    already_done = done
                else:
