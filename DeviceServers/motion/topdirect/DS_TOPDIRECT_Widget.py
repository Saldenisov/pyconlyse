...
class TopDirect_Motor(DS_General_Widget):
    ...
    def register_DS_full(self, group_number=1):
        ...
        setattr(
            self,
            f"l_min_{dev_name}",
            float(ds.get_property("limit_min")["limit_min"][0]),
        )
        setattr(
            self,
            f"l_max_{dev_name}",
            float(ds.get_property("limit_max")["limit_max"][0]),
        )
        setattr(
            self,
            f"name_{dev_name}",
            ds.get_property("friendly_name")["friendly_name"][0],
        )
        setattr(
            self,
            f"preset_pos_{dev_name}",
            list([float(pos) for pos in ds.get_property("preset_pos")["preset_pos"]]),
        )
        l_min, l_max = (
            getattr(self, f"l_min_{dev_name}"),
            getattr(self, f"l_max_{dev_name}"),
        )
        ...
        i = 1
        setattr(
            self, f"radio_button_group_{dev_name}", Qt.QGroupBox("Preset Positions")
        )
        ...
        setattr(
            self,
            f"button_stop_{dev_name}",
            TaurusCommandButton(command="stop_movement"),
        )
