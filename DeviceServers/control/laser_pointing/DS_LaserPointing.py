...
class DS_LaserPointing(DS_ControlPosition):
    RULES = {
        "set_param_after_init": [DevState.ON],
        "start_grabbing": [DevState.ON],
    ...
    def set_optimization_thread(
        self, opt_name: str, actuator_name: str, group_points: str, bounds
    ):
        ...
        optimization_group = Thread(
            target=self.minimization,
            args=[
                list(actuators_devices.values()),
                self.pid_groups[group_points],
                opt_name,
            ],
        )
    ...
    @command(
        dtype_in=str,
        dtype_out=int,
        doc_in="Command that start center of gravity control only for the "
    ...
    def start_cgc(self, client_token=""):
        ...
        if True:
            self.cgc = True
            self.set_optimization_thread(
                "opt1", "Actuators 1", "group1", [(-40, 40), (-40, 40)]
            )
            self.set_optimization_thread(
                "opt2", "Actuators 2", "group2", [(-40, 40), (-40, 40)]
            )
    ...
    @command(
        dtype_in=str,
        dtype_out=int,
        doc_in="Command that stops center of gravity control only for the "
        "locking client, thus it requires to pass client_token.",
    )
    ...
    def minimization(
        self, controlling_devices: List[Device], points_group: str, opt_name: str
    ):
        ...
        def min_func(x):
            ...
            if self.active_opt == opt_name:
                ...
                if (abs(delta_x) <= 2 and abs(delta_y) <= 2) or self.glob_res_n >= 5:
                    ...
                    setattr(
                        self, f"{opt_name}_bounds", [(x_min, x_max), (y_min, y_max)]
                    )
                else:
                    res = (delta_x**2 + delta_y**2) ** 0.5
            else:
                res = 0
    ...
    def execute_action(
        self, action: float, device: Device, relative=False, threaded=False
    ):
        if threaded:
