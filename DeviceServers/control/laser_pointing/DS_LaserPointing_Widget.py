            ...
            def register_ds(device_role, device_name):
                ...
                if state == TaurusDevState.Ready:
                    ...
                    if issubclass(ds_class_widget, OWIS_motor):
                        ds_widget = ds_class_widget(
                            device_name, extra, self, VisType.MIN
                        )
                    ...
                    self.widgets[device_role] = QtWidgets.QLabel(
                        f"Device {device_name} is not ready: {state}."
                    )
                    self.device_servers[device_role] = None
                    print(
                        f"Device {device_name} is not ready. Restart TANGO if NOTHING HELPS!"
                    )
