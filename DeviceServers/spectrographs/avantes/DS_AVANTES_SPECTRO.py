...
class DS_AVANTES_SPECTRO(DS_General):
    ...
    @attribute(
        label="background measurement", dtype=int, access=AttrWriteType.READ_WRITE
    )
    ...
    @attribute(
        label="measure every n sec", dtype=float, access=AttrWriteType.READ_WRITE
    )
    ...
    def _set_channels(self):
        self.signal = spectro_channel(
            ds_name=self.signal_detector, ds=Device(self.signal_detector)
        )
        self.reference = spectro_channel(
            ds_name=self.reference_detector, ds=Device(self.reference_detector)
        )
    ...
    def get_arduino_state(self):
        ...
        try:
            ...
            self.info(
                f"Arduino TTL: {self.ttl_arduino_bool}; Flash Lamp: {self.bg_measurement_value}",
                True,
            )
