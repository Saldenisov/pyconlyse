...
class AVANTES_SPECTRO(DS_General_Widget):
    ...
    def register_DS_full(self, group_number=1):
        ...
        rb_measurement.toggled.connect(
            lambda: self.sync_type(rb_measurement, rb_bg, self.widgets)
        )
        rb_bg.toggled.connect(
            lambda: self.sync_type(rb_measurement, rb_bg, self.widgets)
        )
    ...
    def set_image(self, lo_image):
        ...
        self.add_curve(
            self.plot_od, DataXY(X=np.arange(200, 800, 1), Y=np.zeros(600))
        )  # spectra
    ...
    def sync_type(
        self,
        rb_measure: QtWidgets.QRadioButton,
        rb_bg: QtWidgets.QRadioButton,
        ds_widgets,
    ):
