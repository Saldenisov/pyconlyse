...
if "%INSTANCE_NAME%"=="" (
    ...
    echo Usage: DS_OWIS_Aggregator.bat [instance_name]
    echo Example: DS_OWIS_Aggregator.bat 1
    pause
    exit /b 1
)

echo =====================================================
echo Starting DS_OWIS_Aggregator Device Server
...
set "DS_TITLE=DS_OWIS_Aggregator [%INSTANCE_NAME%]"
where wt >nul 2>&1
if %errorlevel%==0 (
    wt -w PyconlyseTango new-tab --title "%DS_TITLE%" -d "%PYCONLYSE%\DeviceServers\motion\owis" cmd /k "call "%ANACONDA%\Scripts\activate.bat" %PYCONLYSE_ENV% && echo Starting DS_OWIS_Aggregator device server... && %PYCONLYSE_PYTHON% DS_OWIS_Aggregator.py %INSTANCE_NAME%"
) else (
    echo Windows Terminal not found; starting in a separate window...
    start "%DS_TITLE%" cmd /k "cd /d "%PYCONLYSE%\DeviceServers\motion\owis" && "%ANACONDA%\Scripts\activate.bat" %PYCONLYSE_ENV% && echo Starting DS_OWIS_Aggregator device server... && %PYCONLYSE_PYTHON% DS_OWIS_Aggregator.py %INSTANCE_NAME%"
)
