...
where wt >nul 2>&1
if %errorlevel%==0 (
    wt -w 0 nt --title "%DS_TITLE%" -d "%PYCONLYSE%\DeviceServers\motion\owis" cmd /k "call "%ANACONDA%\Scripts\activate.bat" %PYCONLYSE_ENV% && echo Starting DS_OWIS_Aggregator device server... && python DS_OWIS_Aggregator.py %INSTANCE_NAME%"
) else (
    echo Windows Terminal not found; starting in a separate window...
    start "%DS_TITLE%" cmd /k "cd /d "%PYCONLYSE%\DeviceServers\motion\owis" && "%ANACONDA%\Scripts\activate.bat" %PYCONLYSE_ENV% && echo Starting DS_OWIS_Aggregator device server... && python DS_OWIS_Aggregator.py %INSTANCE_NAME%"
)
