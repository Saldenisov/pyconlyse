...
where wt >nul 2>&1
if %errorlevel%==0 (
    wt -w 0 nt --title "%DS_TITLE%" -d "%PYCONLYSE%\DeviceServers\spectrographs\andor" cmd /k "call "%ANACONDA%\Scripts\activate.bat" %PYCONLYSE_ENV% && set PYTHONPATH=%PYCONLYSE% && echo Starting DS_ANDOR_SPECTROGRAPH device server... && python DS_ANDOR_SPECTROGRAPH.py %INSTANCE_NAME%"
) else (
    echo Windows Terminal not found; starting in a separate window...
    start "%DS_TITLE%" cmd /k "cd /d "%PYCONLYSE%\DeviceServers\spectrographs\andor" && "%ANACONDA%\Scripts\activate.bat" %PYCONLYSE_ENV% && set PYTHONPATH=%PYCONLYSE% && echo Starting DS_ANDOR_SPECTROGRAPH device server... && python DS_ANDOR_SPECTROGRAPH.py %INSTANCE_NAME%"
)
