...
class Archive(DS_General_Widget):
    ...
    def register_DS_full(self, group_number=1):
        ...
        layout_data_selection.addSpacerItem(
    ...
    def get_dataset(self, item: QtWidgets.QTreeWidgetItem):
        ...
        order_name = self.ds.get_data(
            [self.dataset_name, f"{from_idx}", average, n_points]
        )
        ready = False
        ...
        if ready:
            if data_string:
                ...
                data = DataXY(
                    X=np.frombuffer(data_d.X, dtype=data_d.Xdtype),
            ...
            print("Did not get response in time.")
    ...
    def set_image(self, lo_image):
        ...
        self.plot = self.view.addPlot(
            title="Spectra", row=0, column=0, axisItems={"bottom": pg.DateAxisItem()}
        )
    ...
    def fill_device_tree_structure(self):
        ...
        self.tree_devices.fill_items(
            self.tree_devices.invisibleRootItem(), device_structure
        )

    def fill_tree_structure(self):
        self.get_structure()
        structure = self.form_main_tree_structure()
        self.tree_structure.fill_items(
            self.tree_structure.invisibleRootItem(), structure
        )
