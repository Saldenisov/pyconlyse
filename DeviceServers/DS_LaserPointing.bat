...
if "%INSTANCE_NAME%"=="" (
    ...
    echo Usage: DS_LaserPointing.bat [instance_name]
    echo Example: DS_LaserPointing.bat 1_LaserPoint_V0
    pause
    exit /b 1
)
...
echo Terminal title: DS_LaserPointing [%INSTANCE_NAME%]
echo.
...
start "%DS_TITLE%" cmd /k "cd /d "%PYCONLYSE%\DeviceServers\control\laser_pointing" && "%ANACONDA%\Scripts\activate.bat" %PYCONLYSE_ENV% && echo Starting DS_LaserPointing device server... && python DS_LaserPointing.py %INSTANCE_NAME%"
...
echo "DS_LaserPointing [%INSTANCE_NAME%]"
echo.
