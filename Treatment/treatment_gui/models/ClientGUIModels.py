"""Forked Client GUI models for the Treatment GUI.

Copied from ``gui.models.ClientGUIModels`` so Treatment-specific
behaviour can evolve independently.
"""
...
class TreatmentModel(QObject):
    ...
    def add_ui_observer(self, in_observer):
        self.ui_observers.append(in_observer)

    def add_data_path(self, file_path: Path, exp_data_type: "TreatmentModel.DataTypes"):
        ...
        if opener:
            ...
            if res:
                ...
                if exp_data_type is TreatmentModel.DataTypes.NOISE:
                    self.noise_averaged_data = np.zeros(shape=(1, 1))
                    self.notify_ui_observers({"lineedit_noise_set": [str(file_path)]})

                elif exp_data_type in [
                    TreatmentModel.DataTypes.ABS,
                    TreatmentModel.DataTypes.ABS_BASE,
    ...
    def add_measurement_observer(self, in_observer):
        self.measurements_observers.append(in_observer)

    def average_range(
        self, line: str, file_path: Path, user_type: str = "kinetics", n: int = 0
    ) -> Union[List[str], None]:
        ...
        if not line_elements:
            self.show_error(
                self.average_range,
                'First fill the required ranges using format: value range;... e.g., "500+-10; 600+-5". '
                "The values and ranges should be given in nm.",
            )
        else:
            opener = self.get_opener(file_path)
            if opener:
                ...
                if user_type == "kinetics":
                    ...
                    variables = info.timedelays
                    x = info.wavelengths
                    file_save = Path(file_path.parent / f"{file_path.stem}_spectra.txt")
                    axis = 0
                else:
                    self.show_error(self.average_range, f"Unsupported user_type: {user_type}")
                    return None
    ...
    def average_noise(self) -> Tuple[bool, str]:
        ...
        if TreatmentModel.DataTypes.NOISE in self.paths:
            ...
            if opener:
                ...
                self.show_error(self.average_noise, "Could not average NOISE")
                res, comments = False, "Could not average NOISE:"
        else:
            res, comments = False, "First add noise path, before averaging"
        return res, comments

    def _callback_average(self, map_index: int, number_maps: int) -> None:
        self.notify_ui_observers({"progressbar_calc": (map_index, number_maps)})

    def calc_abs(self, exp_type: "TreatmentModel.ExpDataStruct", how: str, first_map_with_electrons: bool) -> None:
        res = False
        res_local = True
        if TreatmentModel.DataTypes.NOISE not in self.paths:
            self.show_error(self.calc_abs, 'Set NOISE pass first.')
            res_local = False
        ...
        if exp_type is TreatmentModel.ExpDataStruct.HIS:
            ...
            if TreatmentModel.DataTypes.ABS_BASE not in self.paths:
                self.show_error(self.calc_abs, 'Set ABS_BASE first.')
                res_local = False
            if res_local:
            ...
            if TreatmentModel.DataTypes.ABS not in self.paths:
                self.show_error(self.calc_abs, 'Set ABS pass first.')
                res_local = False
            if TreatmentModel.DataTypes.BASE not in self.paths:
                self.show_error(self.calc_abs, 'Set BASE pass first.')
                res_local = False
            if res_local:
                ...
                if opener:
                    info: CriticalInfo = opener.paths[abs_path]
                    abs_path = self.paths[TreatmentModel.DataTypes.ABS]
                    base_path = self.paths[TreatmentModel.DataTypes.BASE]
                    noise_path = self.paths[TreatmentModel.DataTypes.NOISE]

                    abs_data = self.get_opener(abs_path).average_map(file_path=abs_path,
                                                                     call_back_func=self._callback_average)
                    base_data = self.get_opener(base_path).average_map(file_path=base_path,
                                                                       call_back_func=self._callback_average)
                    noise_data = self.get_opener(noise_path).average_map(file_path=noise_path,
                                                                         call_back_func=self._callback_average)
    ...
    def notify_measurement_observers(self,
        ...
        if opener:
            ...
            if not isinstance(measurement, Measurement):
                ...
                if new:
                    ...
                    cursors = None
                self.notify_measurement_observers(measurement, map_index,
                                                  opener.paths[file_path],
                                                  new=new,
                                                  cursors=cursors)
