@echo off
REM =====================================================
REM TANGO INFRASTRUCTURE - Windows Auto Startup (Fixed)
...
REM Extract port from TANGO_HOST (e.g., 10.20.30.202:10000 -> 10000)
...
start "Tango-Database-Service" cmd /k "cd /d %TANGO_ROOT%\bin && Databaseds.exe 2 -ORBendPoint giop:tcp::%DB_PORT% -poolSize 3"
if %errorlevel% neq 0 (
...
REM Wait for database to initialize using PowerShell (since timeout may not be available)
echo Waiting for database to initialize (15 seconds)...
powershell -Command "Start-Sleep -Seconds 15" 2>nul || (
    echo Waiting using ping method...
...
REM Get hostname using PowerShell or environment variable
...
REM Countdown using PowerShell or ping method
powershell -Command "for($i=10; $i -gt 0; $i--) { Write-Host \"Closing in $i seconds...\"; Start-Sleep -Seconds 1 }" 2>nul || (
    echo Countdown not available, closing...
