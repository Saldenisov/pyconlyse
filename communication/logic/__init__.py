...
class Thinker(ThinkerInter):
    ...
    def __init__(self, parent: dev.Device):
        ...
        self._demands_pending = OrderedDictMesTypeCounter(name='demands_pending')
        self._replies_pending = OrderedDictMesTypeCounter(name='replies_pending')
        self.paused = False
    ...
    def demands_pending_answer(self) -> OrderedDictMod:
        return self._demands_pending

    @property
    def replies_pending_answer(self) -> OrderedDictMod:
        return self._replies_pending
    ...
    def add_demand_pending(self, msg: Message):
        try:
            if len(self._demands_pending) > 1000:
                self._demands_pending.popitem(False)  # pop up last item
            self._demands_pending[msg.id] = PendingDemand(message=msg)
        except KeyError as e:
            info_msg(self, self.add_demand_pending, e)

    def add_reply_pending(self, msg: Message):
        try:
            if len(self._replies_pending) > 1000:
                self._replies_pending.popitem(False)  # pop up last item
            self._replies_pending[msg.id] = PendingReply(message=msg)
        except KeyError as e:
            info_msg(self, self.add_demand_pending, e)
